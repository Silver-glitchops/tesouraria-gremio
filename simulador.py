...
def render() -> None:
    ...
    if not viavel_cap:
        ...
        alerta_borda = "#E67E22"
        alerta_texto = (
            f"⚠️ O evento cobre os custos com {pe_int} participantes, mas para atingir "
        ...
        alerta_texto = (
            ...
            f"lucro de até <strong>R$ {(capacidade_max * margem - custo_fixo):,.2f}</strong> "
            f"com lotação máxima."
        )
    ...
    st.markdown(
        ...
        <div style="background:{alerta_cor};border:1px solid {alerta_borda};
            ...
            <p style="margin:0;font-size:1rem;line-height:1.6;">
                <strong style="font-size:1.4rem;">⚓ {nome_evento}</strong><br>
                {alerta_texto}
            </p>
        </div>
        """,
    ...
    x_max    = max(int(pe_meta * 2), capacidade_max + 10)
    ...
    # Break-even
    fig.add_vline(
        x=pe_pessoas, line_dash="dash",
        line_color="#E8C878", line_width=1.8,
        annotation_text=f"Break-even: {pe_int} pax",
        annotation_position="top right",
        annotation_font_color="#E8C878",
    )

    # Meta
    if meta_lucro > 0:
        fig.add_vline(
            x=pe_meta, line_dash="dash",
            line_color="#9B59B6", line_width=1.5,
    ...
    pontos = sorted(set([
        int(pe_pessoas * m) for m in [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0]
    ] + [pe_int, pe_meta_int, capacidade_max]))
