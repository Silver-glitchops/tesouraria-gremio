...
def render() -> None:
    ...
    # Calcula inadimplentes do mês atual para o badge
    inad_count = 0
...
def _render_tabela_editavel(df: "pd.DataFrame") -> None:
    ...
    # SelectboxColumn para cada mês — elimina erros de digitação
    col_config = {}
    for mes in gs.MESES:
...
def _render_central_cobrancas(df: "pd.DataFrame") -> None:
    ...
    for _, row in df_inad.iterrows():
        ...
        rows_html += f"<tr><td>{nome}</td><td>{len(meses_p)} meses</td><td>{btn}</td></tr>"
